<template>
  ...
  import { fetchBukkData, fetchBukkRawData } from "@/services/apiService";
  import { socketData, initWebSocket } from "@/services/webSocketService";
  ...
  export default {
    ...
    setup() {
      ...
      // **📌 Történelmi adatok betöltése a kiválasztott dátum alapján**
  ...
  try {
    ...
    chartSeries.value[0].data = response.map((item) => ({
      ...
      y: item.powerp !== undefined ? parseFloat(item.powerp.toFixed(2)) : 0, // Ha nincs érték, akkor 0
    }));

    chartSeries.value[1].data = response.map((item) => ({
      x: new Date(item.time).getTime(),
      y: item.irrad !== undefined ? parseFloat(item.irrad.toFixed(2)) : 0, // Ha nincs érték, akkor 0
    }));
    ...
    console.error("Error fetching Bükk data:", error);
      ...
      // **📌 Valós idejű adatok fogadása WebSocketen keresztül**
      const handleRealtimeData = (message) => {
  if (message.event === "data_change" && message.details === "uj_bukk_raw_adat") {
    console.log("📡 WebSocket új adat érkezett:", message);

    const newData = {
      x: new Date(message.timestamp).getTime(),
      y: message.powerp ? parseFloat(message.powerp.toFixed(2)) : 0,
    };
    chartSeries.value[0].data.push(newData);

    const newIrrad = {
      x: new Date(message.timestamp).getTime(),
      y: message.irrad ? parseFloat(message.irrad.toFixed(2)) : 0,
    };
    chartSeries.value[1].data.push(newIrrad);

    // Frissítjük a legutóbbi értékeket
    updateLastValues();
  }
};
      ...
      onMounted(() => {
        ...
        socketData.bukkData.forEach(handleRealtimeData);
      });
